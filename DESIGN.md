...
Iteration Guide
Always use cream (#f7f4ed) as the base — never pure white
Derive grays from #1c1c1c at opacity levels rather than using distinct hex values
Use #eceae4 borders for containment, not shadows
Letter-spacing scales with size: -1.5px at 60px, -1.2px at 48px, -0.9px at 36px, normal at 16px
Two weights: 400 (everything except headings) and 600 (headings)
The inset shadow on dark buttons is the signature detail — don't skip it
Camera Plain Variable at weight 480 is for special display moments only